...
RUN apk add --no-cache -t .build-deps autoconf automake build-base cmake curl git libtool linux-headers perl pkgconf python3-dev re2c tar unzip icu-dev libexecinfo-dev openssl-dev qt5-qtbase-dev qt5-qttools-dev zlib-dev qt5-qtsvg-dev && \
  ...
  cmake --build build && \
  cmake --install build && \
  curl -sSL --retry 5 https://github.com/arvidn/libtorrent/releases/download/v2.0.8/libtorrent-rasterbar-2.0.8.tar.gz | tar xzC /tmp && \
	cd /tmp/*libtorrent* && \
  ...
  cmake --build build && \
  cmake --install build && \
  curl -sSL --retry 5 https://api.github.com/repos/qbittorrent/qBittorrent/tarball/release-4.5.1 | tar xzC /tmp && \
	cd /tmp/*qBittorrent* && \
  cmake -Wno-dev -G Ninja -B build \
    -D CMAKE_BUILD_TYPE="release" \
    -D GUI=OFF \
