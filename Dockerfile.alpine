...
RUN apk add --no-cache -t .build-deps autoconf automake build-base cmake git libtool linux-headers perl pkgconf python3-dev re2c tar unzip icu-dev openssl-dev qt6-qtbase-dev qt6-qttools-dev zlib-dev qt6-qtsvg-dev && \
	apk add --no-cache ca-certificates libressl qt6-qtbase qt6-qtbase-sqlite iptables iptables-legacy openvpn ack bind-tools python3 doas tzdata curl jq && \
  ...
  cmake --build build -j $(nproc) && \
  cmake --install build && \
  curl -sSL --retry 5 https://api.github.com/repos/qbittorrent/qBittorrent/tarball/release-5.1.0 | tar xzC /tmp && \
	cd /tmp/*qBittorrent* && \
  cmake -Wno-dev -G Ninja -B build \
    -D CMAKE_BUILD_TYPE="release" \
    -D GUI=OFF \
